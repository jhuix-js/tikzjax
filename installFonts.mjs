...
https.get('https://us.mirrors.cicku.me/ctan/fonts/cm/ps-type1/bakoma.zip', (response) => {
  ...
  file.on('finish', async () => {
    ...
    const files = await decompress(zipFile, tmpDir, {
      filter: (file) =>
        path.extname(file.path) === '.otf' || path.basename(file.path) === 'LICENCE',
    });
    fs.rmSync(path.join(import.meta.dirname, 'css', 'fonts'), { recursive: true });
    fs.mkdirSync(path.join(import.meta.dirname, 'css', 'fonts', 'woff'), { recursive: true });
    for (const file of files) {
      const basename = path.basename(file.path);
      if (basename === 'LICENCE') {
        fs.copyFileSync(
          path.join(tmpDir, file.path),
          path.join(import.meta.dirname, 'css', 'fonts', 'LICENSE'),
        );
        continue;
      }
